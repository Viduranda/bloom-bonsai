...
$allowedOrigin = rtrim((string)(getEnvVar('APP_ORIGIN', '*')), '/');
...
// Tier 1: Try Primary Database Config
if (!empty($pass)) {
    try {
...
// Tier 2: Try AlwaysData Local Host if Tier 1 failed
if (!$connected) {
    $alwaysUser = getEnvVar('MYSQL_USER', 'bloom-bonsai');
    $alwaysPass = getEnvVar('MYSQL_PASSWORD', '');
    $alwaysDb   = getEnvVar('MYSQL_DB', 'bloom-bonsai_db');
    $alwaysHost = getEnvVar('MYSQL_HOST', 'mysql-bloom-bonsai.alwaysdata.net');

    try {
        $pdo = new PDO("mysql:host=$alwaysHost;dbname=$alwaysDb;charset=utf8mb4", $alwaysUser, $alwaysPass, [
            PDO::ATTR_ERRMODE            => PDO::ERRMODE_EXCEPTION,
            PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,
            PDO::ATTR_EMULATE_PREPARES   => false,
        ]);
...
// Tier 3: Try Local 127.0.0.1 XAMPP Database
if (!$connected) {
    try {
...
// Tier 4: Fallback Cloud InfinityFree Credentials
if (!$connected) {
    try {
        ...
        $connected = true;
    } catch (PDOException $infErr) {
        http_response_code(500);
        echo json_encode(['success' => false, 'error' => 'Database connection failed: Unable to connect to MySQL database server.']);
        exit;
    }
}
